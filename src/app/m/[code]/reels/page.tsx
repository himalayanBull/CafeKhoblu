...
function ReelCard({ dish, isActive, code }: { dish: typeof dishes[0]; isActive: boolean; code: string }) {
  ...
  return (
    <div className="relative h-full w-full bg-stone-950">
      ...
      <div className="absolute right-4 bottom-32 flex flex-col items-center gap-5">
        ...
        <Link href={`/m/${code}/dish/${dish.id}`} className="flex flex-col items-center gap-1.5">
          <div className="h-12 w-12 rounded-full bg-white/10 backdrop-blur-sm flex items-center justify-center">
...
export default function ReelsPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  return (
    <div ref={containerRef} className="h-[100dvh] w-full overflow-hidden bg-black relative">
      <div className="absolute top-0 left-0 right-0 z-20 p-5 flex items-center justify-between">
        <Link href={`/m/${code}`} className="text-white/60 text-sm font-medium hover:text-white transition-colors">
          ← Menu
        </Link>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <ReelCard dish={dishes[currentIndex]} isActive={true} code={code} />
        </motion.div>
      </AnimatePresence>
