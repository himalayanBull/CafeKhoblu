...
export default function MenuPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const table = menuCode.tableNumber.toString();

  return <MenuContent code={code} table={table} />;
}

function MenuContent({ code, table }: { code: string; table: string }) {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF9]">
      ...
      {filters.category === 'all' && !filters.searchQuery && (
        <>
          <DiscoverySections basePath={`/m/${code}`} />

          {/* Reels CTA */}
          <div className="px-5 mb-8">
            <Link href={`/m/${code}/reels`}>
      ...
      <div className="px-5 pb-36">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {filteredDishes.map((dish, i) => (
            <DishCard key={dish.id} dish={dish} index={i} basePath={`/m/${code}`} />
          ))}
        </div>
